...
class BuilderMixin():
    ...
    def __init__(self, master, geometry, **options):
    ...
    def smart_builder(self, tkClass, geometry, **options):
        ''' Build new widget on self '''
        class WidgetBuilder(BuilderMixin, tkClass):
            pass

        # Parse geometry and instantiate widget
        match = re.fullmatch(self.GEOM_RE, geometry)
        if not match:
            raise Exception('Wrong geometry options')

        specified = \
            {k: v for k, v in match.groupdict().items() if v is not None}
        return WidgetBuilder(self, specified, **options)
...
class Application(tk.Frame, BuilderMixin):
